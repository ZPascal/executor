// GPUManager discovers GPU devices on this cell via nvidia-smi, maintains an
// allocation table (GPU index -> container handle), and exposes
// Allocate/Release/Available/TotalGPUs for the container create/destroy
// lifecycle in depot/containerstore. Instantiated once at executor startup
// (see initializer/initializer_garden.go).
package executor
...
func (m *GPUManager) Allocate(containerHandle string, count int) ([]uint, error) {
	...
	if m.allocated == nil {
		m.allocated = make(map[uint]string)
	}
